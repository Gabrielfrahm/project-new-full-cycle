...
describe('CategoryFakerBuilder Unit Tests', () => {
  describe('category_id prop', () => {
    ...
    test('should pass index to category_id factory', () => {
      ...
      const uuid = new Uuid();
      mockFactory = jest.fn(() => uuid);
      const fakerMany = CategoryFakeBuilder.theCategories(2);
      fakerMany.withCategoryId(mockFactory);
      fakerMany.build();

      expect(mockFactory).toHaveBeenCalledTimes(2);
      expect(fakerMany.build()[0].category_id).toBe(uuid);
      expect(fakerMany.build()[1].category_id).toBe(uuid);
    });
  });
  ...
  test('should create a category', () => {
    ...
    expect(category.is_activity).toBe(true);
    ...
    expect(category.is_activity).toBe(false);
